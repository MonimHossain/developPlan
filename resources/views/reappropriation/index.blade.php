                        ...
                        <form action="" method="post" class="form-horizontal">
                            ...
                            <div class="span12 margin-bottom">
                                    <div  class="span12">
                                        ...
                                        <div class="span6">
                                            <div class="span12">
                                                <label class="control-label span4"> @lang("Targets") :</label>
                                                <div class="controls">
                                            ...
                                            <div class="span12" style="margin-left:-2px">
                                                <label  class="control-label span4"> @lang("Select Projects") :</label>
                                                <div class="controls">
                                                    <select name="call_notice_type" onchange="get_next_demand_data()" id="next_proj_name" class="span8">
                                                    <option></option>
                                                    @foreach($projects as $key => $pro)
                                                    <option value="{{$pro->project_name->unapprove_project_id}}">{{$pro->project_name->project_title}}</option>
                                                    @endforeach
                            ...
                            <br/>
                            <fieldset>
                                <legend>@lang('Re-Appropriation')</legend>
                                <div class="span12">
                                    <div class="span12 margin-bottom" >
                                        <div class="span4">
                                            <label for="" class="span5" >@lang('Project Name'):</label>
                            ...
                            <div class="row-fluid">
                                <div class="controls" align="center" style="margin-left:-50px">
                                    <input type="button" onclick="get_reappropriation_data()" id="btnSendData" name="btnSendData" value="@lang('Add More')"  />
                                </div>
                            </div>
                            </form>
                            </div>
            ...
            <table class="table table-bordered table-striped" id="source_table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th colspan="6">@lang('ADP Appropriation')</th>
                                <th colspan="2">@lang("Adjust amount of money")</th>
                                <th colspan="6">@lang("Revised Appropriation")</th>
                            </tr>
                            <tr>
...
<script  type="text/javascript">
    ...
    var reAppropriation = new Array();
    var rowCount;
    
    function get_next_demand_data(flag=0){
    
    if(flag == 0)
        var pro_name = $('#next_proj_name').val();
    else{   
        pro_name = flag;
        $('#next_proj_name').val(pro_name);
    }
    $.ajax({
    url: "{{url('demand_data')}}",
            type: "get",
    ...
    function get_reappropriation_data(){
        ...
        if($('#next_proj_name').val()==""){
            alert("Can't process with empty project row.");
            return $('#next_proj_name').focus();
        }
        if($('#re_total').val()==""){
        ...
        var target_name = $('#proj_name').val();
        var pro_name = $('#next_proj_name').val();
        var originalRevenue = $('#original_revenue').val();
        var reAppTotal = $('#re_total').val() == "" ? 0 : $('#re_total').val();
        var reAppTaka = $('#re_taka').val()== "" ? 0 : $('#re_taka').val();
        var reAppRevenue = $('#re_revenue').val()== "" ? 0 : $('#re_revenue').val();
        var reAppCapital = $('#re_capital').val()== "" ? 0 : $('#re_capital').val();
        var reAppCapitalRpa = $('#re_capital_rpa').val()== "" ? 0 : $('#re_capital_rpa').val();
        var reAppCapitalRevenue = $('#re_capital_revenue').val()== "" ? 0 : $('#re_capital_revenue').val();
        var reAppCdVat = $('#re_cdvat').val()== "" ? 0 : $('#re_cdvat').val();
        var reAppPa = $('#re_cdvat_pa').val()== "" ? 0 : $('#re_cdvat_pa').val();
        var reAppPaRpa = $('#re_cdvat_rpa').val()== "" ? 0 : $('#re_cdvat_rpa').val();
        var reAppOthers = $('#re_other').val()== "" ? 0 : $('#re_other').val();
        
          
        originalRevenue = parseFloat(originalRevenue);
        reAppRevenue = parseFloat(reAppRevenue);
        
        if(originalRevenue < reAppRevenue){
            var increase = reAppRevenue - originalRevenue;
            var decrease = 0;
        }
        else{
            var decrease = originalRevenue - reAppRevenue;
            var increase = 0;
        }
        
        reAppropriation[rowCount-1] = [pro_name, target_name, reAppTotal,
                                      reAppTaka, reAppRevenue,reAppCapital,
                                      reAppCapitalRpa, reAppCapitalRevenue,
                                      reAppCdVat, reAppPa, reAppPaRpa,
                                      reAppOthers, deleteFlag];
        var flag = 0;
        if(rowCount>1){
            var i = 0;
            while(i < rowCount-1){
                if(reAppropriation[i][0] == pro_name && reAppropriation[i][12] == 0){
                    reAppropriation = reAppropriation.slice(0);
                    reAppropriation.splice(rowCount-1, 1);
                    flag = 1;
                    console.log(reAppropriation);
...
//        $('#sl_no_'+rowCount).append($('#alloc_realloc tr').length);
        $('#pr_name_'+rowCount).append($('#re_project_name').val());
...
//                    +'<td id="sl_no_'+ rowCount + '"></td>'
    ...
    function get_table_data(){
        ...
        if($('#alloc_realloc tr').length > 1){
            rowCount = $('#alloc_realloc tr').length;
            $.ajax({
            url: "{{url('reAppropriationDataSet')}}",
                type: "get",
                data: {value : reAppropriation},
                success: function (data){
                 console.log(data);
                },
            });
